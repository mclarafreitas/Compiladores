...
Token Lexer::Scan()
{
    ...
    if (peek == '/')
    {
        ...
        if (peek == '/')
        {
            ...
            while (peek != '\n' && peek != EOF)
                peek = cin.get();
            return Scan(); // recursivamente procura pelo próximo token
        }
        else if (peek == '*')
        {
        ...
        {
            // trata como operador de divisão
            Token t{ '/' };
            cout << "<" << char(t.tag) << "> ";
            return t;
        }
...
void Lexer::Start()
{
    while (true)
    {
        Token t = Scan(); // chama o Scan para pegar um token
        if (cin.eof()) // verifica se a entrada chegou ao fim
            break;
    }
}
