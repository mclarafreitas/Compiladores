...
{
    if (!expr1 || !expr2 || !token)
        throw SyntaxError(scanner->Lineno(), "Expressões ou token inválido");

    // Conversão de int para bool
    if (expr1->type == ExprType::INT) {
        expr1->type = ExprType::BOOL;
        expr1->token->lexeme = (expr1->token->lexeme == "0") ? "false" : "true";
    }
    if (expr2->type == ExprType::INT) {
        expr2->type = ExprType::BOOL;
        expr2->token->lexeme = (expr2->token->lexeme == "0") ? "false" : "true";
    }
...
{
    if (!expr || !token)
        throw SyntaxError(scanner->Lineno(), "Expressão ou token inválido");
...
{
    if (!id || !expr)
        throw SyntaxError(scanner->Lineno(), "Identificador ou expressão inválido");
...
If::If(Expression *e, Statement *s) 
    : Statement(NodeType::IF_STMT), expr(e), stmt(s)
{
    if (!expr || !stmt)
        throw SyntaxError(scanner->Lineno(), "Expressão ou statement inválido");
