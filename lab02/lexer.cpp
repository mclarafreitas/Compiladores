...
Token * Lexer::Scan()
{
	...
	while (peek == '/')
	{
		...
		else if (peek == '*')
		{
			...
			while(true)
			{
				...
				{
					token = Token{EOF, ""}; // EOF token com lexema vazio
					return &token;
				}
			}
			peek = fin.get();
	...
	// retorna caracteres não alphanuméricos isolados
	token = Token(int(peek), std::string(1, peek)); // corrigido para construir Token adequadamente
	peek = ' ';
	return &token;
}
