...
While::While(Expression *e, Statement *s) : 
    ...
    stmt(s) 
{
    // Cria novos rótulos de entrada e saída
    begin = NewLabel();
    after = NewLabel();
}
...
void While::Gen()
{
    cout << 'L' << begin << ':' << endl; // Rótulo de início do laço
    Expression *n = Rvalue(expr); // Avaliação da expressão condicional
    cout << "\tifFalse " << n->ToString() << " goto L" << after << endl; // Desvio para o fim do laço se a condição for falsa
    stmt->Gen(); // Geração do corpo do laço
    cout << "\tgoto L" << begin << endl; // Volta ao início do laço
    cout << 'L' << after << ':' << endl; // Rótulo de saída do laço
}
